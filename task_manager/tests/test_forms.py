...
from task_manager.models import Team, Worker, Task, TaskType


class AddWorkerFormTest(TestCase):
    def setUp(self):
        self.task_type_instance = TaskType.objects.create(name="Test")
        self.task_instance = Task.objects.create(
            deadline=datetime.now(), task_type=self.task_type_instance
        )
    ...
    def test_init_fields(self):
        ...
        self.assertIsInstance(
            form.fields["workers_no_team"], forms.ModelMultipleChoiceField
        )
        self.assertIsInstance(
            form.fields["select_all_team"],
            forms.BooleanField
        )
        for team in Team.objects.all():
            field_name = f"workers_{team.id}"
            ...
            if self.task_instance:
                ...
                self.assertEqual(
                    list(form.fields[field_name].initial),
    ...
    def test_clean_method(self):
        form_data = {
            "workers_no_team": [
                worker.id for worker
                in Worker.objects.filter(team__isnull=True)
            ],
            "select_all_team": True,
        }
        for team in Team.objects.all():
            form_data[f"workers_{team.id}"] = [
                worker.id for worker in Worker.objects.filter(team=team)
            ]
